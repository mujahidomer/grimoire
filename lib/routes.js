...
const { listChats, listMessages, deleteChat } = require('./chatHistory');
...
function registerApiRoutes(app) {
  ...
  // GET /api/stats -- count-only footer stats for the iOS Search tab: total
  // saved links, digest-bearing items, and top-category count.
  app.get('/api/stats', requireAuth(async (req, res) => {
    try {
      res.json(await getStats({ userId: req.userId }));
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  }));
  ...
  // DELETE /api/chats/:chatId — remove a conversation and its messages.
  app.delete('/api/chats/:chatId', requireAuth(async (req, res) => {
    try {
      await deleteChat(req.userId, req.params.chatId);
      res.json({ success: true });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
