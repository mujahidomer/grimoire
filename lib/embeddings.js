...
async function vectorSearch({ userId, queryEmbedding, threshold }) {
  const sb = getSupabase();
...
function mergeSearchResults(vectorRows, keywordRows) {
  ...
  if (orderedIds.length < MERGED_POOL_CAP) {
...
async function rerankWithHaiku(query, candidates) {
  ...
  const listing = candidates.map((item, index) => {
    const takeaways = !isEmptyTakeaways(item.key_takeaways)
      ? takeawaysToText(item.key_takeaways)
      : '(none)';
    return [
      `[${index}]`,
...
function fallbackByVectorSimilarity(candidates) {
  return candidates
    .filter(item => item.similarity != null)
    .sort((a, b) => (b.similarity || 0) - (a.similarity || 0))
    .slice(0, FALLBACK_VECTOR_TOP);
}
...
async function semanticSearch({ userId, query, threshold } = {}) {
  const q = (query || '').trim();
  if (!q) return { items: [], queryEmbedding: null };

  const queryEmbedding = await embedQuery(q);
  const [vectorRows, keywordRows] = await Promise.all([
    vectorSearch({ userId, queryEmbedding, threshold }),
    keywordSearch(userId, q)
  ]);

  const { orderedIds, simById } = mergeSearchResults(vectorRows, keywordRows);
  ...
  const candidates = orderedIds
    .map(id => {
      ...
      if (!item) return null;
      return { ...item, similarity: simById.get(id) ?? null };
    })
    .filter(Boolean);
  ...
  try {
    ...
    reranked = fallbackByVectorSimilarity(candidates);
  }
