...
function buildFrontMatterObject(item) {
  ...
  // skills (skill-type saves only) renders right after artifact_url_source.
  if (Array.isArray(item.skills) && item.skills.length > 0) {
    front.skills = item.skills;
  }

  front.linked_resources = linkedResources;
  return front;
}
...
function buildBody(item) {
  ...
  const takeawaysValue = item.key_takeaways ?? item.keyTakeaways;
  const takeaways = !isEmptyTakeaways(takeawaysValue)
    ? takeawaysToMarkdown(takeawaysValue)
    : '_No key takeaways._';
