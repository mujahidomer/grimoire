...
const SUBFOLDERS = ['Skills', 'Markdown', 'Agents', 'Insights', 'Installed'];
...
async function initialize() {
  ...
  if (!rootFolderId) throw new Error('DRIVE_ROOT_FOLDER_ID is not set in environment variables');
...
async function getOrCreateSheet(drive, sheets, parentId) {
  ...
  // Create sheet inside the Grimoire folder
  ...
  await sheets.spreadsheets.values.update({
    ...
    requestBody: {
      values: [[
        'Skill Name', 'Type', 'Category', 'Description',
        'Source URL', 'Date Saved', 'Status', 'Drive File Link'
      ]]
    }
  });
