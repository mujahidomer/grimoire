...
async function initialize() {
  ...
  await ensureTagReviewsSheet(sheets, sheetId);
...
async function ensureTagRegistrySheet(sheets, sheetId) {
  await ensureTab(sheets, sheetId, 'Tag Registry', ['Tag', 'Added Date', 'File Count']);
}

// Idempotent: create the 'Tag Reviews' tab that maps a tag-notification's Telegram
// message id back to the file + flagged tag, so replies can be resolved later.
async function ensureTagReviewsSheet(sheets, sheetId) {
  await ensureTab(sheets, sheetId, 'Tag Reviews', ['Message ID', 'Drive File ID', 'Flagged Tag', 'Resolved']);
}

// Create a tab with a bold header row if it does not already exist.
async function ensureTab(sheets, sheetId, title, headers) {
