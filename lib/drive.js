...
// 0 means "not a clear primary" (generic/secondary link).
function primaryLinkRank(url) {
  ...
  if (host === 'github.com' || host.endsWith('.github.io')) return 3; // repo / project page
  if (host.startsWith('docs.') || host.startsWith('developer.') || /\/docs(\/|$)/.test(path)) return 2; // official docs
  if (path === '/' || path === '') return 1; // product homepage
  return 0; // generic / secondary
}

// Pick the most likely primary link for an artifact from a list of candidates,
// preferring GitHub repos, then official docs, then homepages. Returns null when
// no candidate is a clear primary (so we never promote a generic/secondary link).
