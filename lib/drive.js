...
// Strip markdown link wrapping — "[text](url)" → "url" — and trim. Plain strings
// pass through unchanged. Returns null for empty/missing values.
...
function buildFrontMatterObject(item) {
  // Normalized tags (may carry a '?' low-confidence flag) take precedence so the
  // frontmatter reflects the canonical/flagged set; hashtags are the fallback.
  // Kept as a comma-joined scalar so the tag-review resolver can parse it.
  ...
  const artifactUrl = artifactType !== 'none' ? cleanUrl(item.artifact_url) : null;
  ...
  const linked = (Array.isArray(item.linked_resources) ? item.linked_resources : [])
    .map(cleanUrl)
    .filter(Boolean);
  const linkedResources = [...new Set(linked)].filter(u => u !== artifactUrl);
...
function buildBody(item) {
  ...
  const takeaways = Array.isArray(item.keyTakeaways) && item.keyTakeaways.length > 0
...
// frontmatter `tags:` line. Dedupes, and reports which '?' tags remain so callers
// can decide whether the file still needs review.
function replaceTagInContent(content, flaggedTag, canonicalTag) {
  ...
  const current = lines[idx].replace(/^tags:\s*/, '').split(',').map(t => t.trim()).filter(Boolean);
