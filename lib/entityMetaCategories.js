...
const META_CATEGORIES = {
  ...
  //
  // (hadith joined 2026-07-24 when the app's sweep first registered its
  // rows — per the standing decision that it shares these shelves rather
  // than getting a vocabulary of its own.)
  dua: ISLAMIC_SHELVES,
  hadith: ISLAMIC_SHELVES,
  islamic_concept: ISLAMIC_SHELVES,
  quranic_verse: ISLAMIC_SHELVES,
  // Places split by their NATURE (what kind of spot it is), same principle as
  // every other type. The iOS Travels page ALSO groups these rows by
  // destination — but destination is a fact (detail.country from extraction /
  // geocoding), not a shelf, so it never appears in this vocabulary.
  place: [
    'Food & Dining',
    'Sights & Landmarks',
    'Nature & Outdoors',
...
const LABEL_DEFAULT = {
  ...
  place: {
    ...
    'cafes': ['cafés', 'Food & Dining'],
    'dining': ['restaurants', 'Food & Dining'],
    'beaches': ['beaches', 'Nature & Outdoors'],
    'parks': ['parks', 'Nature & Outdoors'],
    'hiking': ['hikes & trails', 'Nature & Outdoors'],
    'landmarks': ['landmarks', 'Sights & Landmarks'],
    'attractions': ['attractions', 'Sights & Landmarks'],
    'museums': ['museums', 'Sights & Landmarks'],
    'hotels': ['hotels', 'Stays & Retreats'],
    'resorts': ['resorts', 'Stays & Retreats'],
    'markets': ['markets & souks', 'Shopping & Markets'],
    'shopping': ['markets & souks', 'Shopping & Markets'],
  },
  workflow: {
