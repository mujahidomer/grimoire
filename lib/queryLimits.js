...
// Comma-separated allowlist in env. QUERY_LIMIT_BYPASS_EMAILS is resolved via the
// Supabase admin API; QUERY_LIMIT_BYPASS_USER_IDS matches auth user UUIDs directly.
...
function parseBypassEmails() {
  return parseCsvEnv('QUERY_LIMIT_BYPASS_EMAILS').map((email) => email.toLowerCase());
}

function parseBypassUserIds() {
  return parseCsvEnv('QUERY_LIMIT_BYPASS_USER_IDS');
}

const bypassEmailByUserId = new Map();

async function isLimitBypassed(userId) {
  if (parseBypassUserIds().includes(userId)) return true;

  const emails = parseBypassEmails();
  if (!emails.length) return false;

  if (!bypassEmailByUserId.has(userId)) {
    const sb = getSupabase();
    const { data, error } = await sb.auth.admin.getUserById(userId);
    const email = error || !data?.user?.email ? null : data.user.email.toLowerCase();
    bypassEmailByUserId.set(userId, email);
  }

  const email = bypassEmailByUserId.get(userId);
...
async function getUsage(userId) {
  if (await isLimitBypassed(userId)) {
    return {
      queryCount: 0,
      deepCount: 0,
...
async function incrementUsage(userId, isDeep) {
  if (await isLimitBypassed(userId)) return;
