...
Schema:
- items (id uuid, user_id uuid, title text, source_url text, top_category text, topic_subcategory text, summary text, caption text, created_at timestamptz)
- top_category is one of 13 fixed labels (e.g. 'Technology & AI', 'Religion & Spirituality'); topic_subcategory is free text. There is NO column named category.
...
- To query individual entities, ALWAYS use exactly this subquery pattern:
  SELECT sub.id, sub.title, sub.source_url, sub.entity->>'name' AS entity_name
  FROM (SELECT i.*, jsonb_array_elements(i.entities) AS entity FROM items i WHERE i.user_id = '<userId>') sub
  WHERE sub.entity->>'type' = 'tool' LIMIT 200
  Outside the subquery the inner alias i does NOT exist — qualify every outer column with sub. (sub.title, sub.entity->>'name', never i.title).
...
8. For questions about saved tools, apps, or websites, list tool entities: use the jsonb_array_elements subquery pattern over entities with entity->>'type' = 'tool', selecting entity->>'name' AS tool_name alongside the parent item's id, title, source_url
9. Respond with JSON only: {"sql": "SELECT ..."}`;
...
- Format for readability: a GitHub-flavored markdown table when the rows are a comparison or a multi-column breakdown; ### section headings for multi-part answers. Leave a blank line between sections.
...
function buildAnswerPrompt(question, rows, sources, historyText = '') {
  // The raw rows carry uuids, urls, and repeated titles the model never uses
  // (citations are [[N|Title]] against the numbered list) — at 100 rows the
  // pretty-printed original was ~10k input tokens. Rows that map to a source
  // are collapsed to a `ref` back-pointer; the rest keep their data columns.
  const refById = new Map(sources.map((s, i) => [s.id, i + 1]));
  const slimRows = rows.map((row) => {
    if (!row || typeof row !== 'object') return row;
    const { id, item_id, title, source_url, sourceUrl, ...rest } = row;
    const ref = refById.get(id ?? item_id);
    if (ref) {
      // Keep an aliased data column that merely reuses the name `title`
      // (e.g. entity name AS title) — only drop it when it duplicates the
      // source list entry.
      const srcTitle = sources[ref - 1] && sources[ref - 1].title;
      return (typeof title === 'string' && title && title !== srcTitle)
        ? { ref, title, ...rest }
        : { ref, ...rest };
    }
    if (typeof title === 'string' && title) return { title, ...rest };
    if (id ?? item_id) return { id: id ?? item_id, ...rest };
    return rest;
  });

  let content = `${historyText}Question: ${question}\n\n`;
  if (sources.length > 0) {
    ...
    content += '\n';
    content += 'In the rows below, `ref` is the item number [N] in the list above.\n';
  }
...
async function answerLibraryQuery({ question, userId, historyText = '', write, end } = {}) {
  ...
  try {
    ...
    const stream = client.messages.stream({
      ...
      messages: [{
        role: 'user',
        content: buildAnswerPrompt(question, rows, sources, historyText)
      }]
    });
