...
// The syndication payload attaches a link-card preview (title/description) to a
// tweet that shares an external URL. We use it as fallback content when the
// linked article body can't be fetched, so the classifier sees real substance
// instead of a bare shortened URL. Returns '' if there's no card or the shape is
// unexpected — defensive by design, never throws.
function cardPreviewText(data) {
  const bindings = data && data.card && data.card.binding_values;
  if (!bindings) return '';
  const read = (key) => {
    // binding_values may be an object map or an array of { key, value }.
    const v = Array.isArray(bindings)
      ? (bindings.find((b) => b && b.key === key) || {}).value
      : bindings[key];
    return (v && (v.string_value || v.value) || '').trim();
  };
  return [read('title'), read('description')].filter(Boolean).join('\n\n');
}
...
async function extractTwitter(url) {
  ...
  // bare URL — fall back to a standard tweet save, folding in the syndication
  // link-card preview when present so there's real content to classify. The
  // tweet text stays the caption either way.
  const hadLink = TCO_RE.test(text);
  const preview = hadLink ? cardPreviewText(data) : '';
  if (hadLink) {
    console.log(preview
      ? `[extractTwitter] Article body unavailable for ${url}; using syndication card preview (${preview.length} chars)`
