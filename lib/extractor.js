 ...
 *   - Twitter/X → oEmbed (text/author, no auth) + Supadata for embedded video
 *     transcript + Haiku vision for photo tweets
...
// break the bare Supadata path. oEmbed gives us the tweet body + author with no
// auth, and we layer Supadata (video) / Haiku vision (photos) on top.

// Strip HTML tags and decode the handful of entities oEmbed emits, turning the
// blockquote markup into the plain tweet text (incl. the trailing
// "— Author (@handle) date" line).
...
// A tweet is treated as a thread when the path carries more than one status id
// or the URL has the `?s=` parameter. NOTE: `?s=` is the share-tracking param
// present on most copied tweet links, so this intentionally over-flags per the
// current spec — we only annotate, we don't parse the thread yet.
function isThreadUrl(url) {
  try {
    const u = new URL(url);
    if (u.searchParams.has('s')) return true;
    const ids = u.pathname.match(/\/status(?:es)?\/\d+/g) || [];
    return ids.length > 1;
  } catch {
    return false;
  }
...
async function extractTwitter(url) {
  ...
  console.log(`[extractTwitter] Fetching oEmbed for: ${url}`);

  const res = await fetch(endpoint, { headers: { Accept: 'application/json' } });
  if (!res.ok) throw new Error(`Twitter oEmbed failed (${res.status}) for ${url}`);
  ...
  // Video tweet: keep the oEmbed text and add the Supadata transcript on top.
  ...
  // Text-only tweet.
  return { text, author, transcript: null, image_descriptions: null, type: 'tweet_text', thread };
}
...
export async function extractContent(url) {
  ...
  if (isTwitterUrl(url)) {
    console.log(`[extractContent] Routing ${url} → Twitter (oEmbed)`);
