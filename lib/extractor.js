...
async function extractMeta(url) {
  ...
  if (isInstagram) {
    ...
    if (audioUrl) {
      try {
        transcript = await transcribeWithWhisper(audioUrl);
        console.log(`[extractMeta] Whisper transcription succeeded (${transcript.length} chars)`);
      } catch (err) {
    ...
    const text = [transcript, caption].filter(Boolean).join('\n\n');
