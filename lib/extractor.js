...
async function extractMeta(url) {
  ...
  if (isInstagram) {
    ...
    if (postType === 'Video') {
      ...
      if (audioUrl) {
        try {
          visionText = await transcribeWithWhisper(audioUrl);
          console.log(`[extractMeta] Whisper transcription succeeded (${visionText.length} chars)`);
        } catch (err) {
      ...
      const imageUrls = (
        Array.isArray(item.images) && item.images.length > 0
          ...
          : (Array.isArray(item.childPosts) ? item.childPosts.map(p => p.displayUrl) : [])
      ).filter(Boolean);
