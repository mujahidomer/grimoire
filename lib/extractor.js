...
const anthropic = anthropicClient.getAnthropicClient();
...
// or, for X's own long-form Articles, through Apify (see below).
// Returns the article body, or null if there's no t.co link or expansion fails.
const TCO_RE = /https?:\/\/t\.co\/[A-Za-z0-9]+/;

// X Articles (long-form posts) are the one t.co target Jina can never read:
// x.com serves a bare JS shell to anonymous fetchers and the body only renders
// behind a logged-in session, so Jina returns 403/451 every time. The X API
// doesn't help either — its `article` field carries the title and nothing else.
// This actor renders the Article and returns the body as markdown, keyed by the
// tweet id (NOT the article id, which we never have to parse out).
//
// parsebird over the (cheaper-looking) fastcrawler actor deliberately: fastcrawler
// pads every dataset to 10 rows and bills per row, so a single article costs
// $0.11 there vs $0.006 here, and it bills the same $0.11 when it returns nothing.
const X_ARTICLE_ACTOR = process.env.APIFY_ARTICLE_ACTOR
  || 'parsebird~x-twitter-article-markdown';

// The actor renders a logged-in page, so it intermittently comes back SUCCEEDED
// with an empty `md` (observed ~1 in 3). One retry clears it; billing is per
// non-empty result, so a retry after an empty run is free.
const X_ARTICLE_ATTEMPTS = 2;

async function extractTwitterArticle(tweetId) {
  let lastErr;
  for (let attempt = 1; attempt <= X_ARTICLE_ATTEMPTS; attempt++) {
    try {
      const md = await runArticleActor(tweetId);
      if (md) return md;
      console.warn(`[extractTwitter] X Article actor returned empty markdown for ${tweetId} (attempt ${attempt}/${X_ARTICLE_ATTEMPTS})`);
    } catch (err) {
      lastErr = err;
      console.warn(`[extractTwitter] X Article actor failed for ${tweetId} (attempt ${attempt}/${X_ARTICLE_ATTEMPTS}): ${err.message}`);
    }
  }
...
async function runArticleActor(tweetId) {
  const token = process.env.APIFY_API_KEY;
  if (!token) {
    console.warn('[extractTwitter] APIFY_API_KEY not set, cannot fetch X Article body');
    return null;
  }

  const base = 'https://api.apify.com/v2';
  console.log(`[extractTwitter] Starting Apify actor ${X_ARTICLE_ACTOR} for tweet ${tweetId}`);

  const runRes = await fetch(`${base}/acts/${X_ARTICLE_ACTOR}/runs?token=${token}`, {
    method: 'POST',
  ...
  if (!runRes.ok) {
    throw new Error(`Apify article run start failed (${runRes.status}): ${await runRes.text()}`);
  }

  const { data: run } = await runRes.json();
  const runId = run.id;

  // The actor finishes in ~3s, so poll faster and for far less time than the
  // Meta scrapers (2s × 30 = 60s ceiling) — a save shouldn't hang on this.
  let status = run.status;
  for (let i = 0; i < 30 && status !== 'SUCCEEDED' && status !== 'FAILED' && status !== 'ABORTED'; i++) {
    await new Promise(r => setTimeout(r, 2000));
    const pollRes = await fetch(`${base}/actor-runs/${runId}?token=${token}`);
    if (!pollRes.ok) throw new Error(`Apify article poll failed (${pollRes.status})`);
    ({ data: { status } } = await pollRes.json());
  }

  if (status !== 'SUCCEEDED') {
    throw new Error(`Apify article run ${runId} ended with status: ${status}`);
  }

  const itemsRes = await fetch(`${base}/actor-runs/${runId}/dataset/items?token=${token}`);
  if (!itemsRes.ok) throw new Error(`Apify article dataset fetch failed (${itemsRes.status})`);
  const items = await itemsRes.json();

  // Find the row carrying `md` rather than indexing items[0] — some article
  // actors pad the dataset with placeholder rows, and an empty `md` is the
  // actor's own "couldn't render it" signal, which the caller retries.
  const hit = (Array.isArray(items) ? items : []).find(i => typeof i?.md === 'string' && i.md.trim());
  return hit ? hit.md.trim() : null;
}

async function resolveTcoArticle(text, tweetId) {
  ...
  // X Articles go to Apify, not Jina — see extractTwitterArticle. Without
  // a tweet id there's nothing to key the actor on, so fall through to the
  // preview excerpt rather than burning a Jina call that's certain to 403.
  if (isTwitterArticle) {
    if (!tweetId) return null;
    try {
      const body = await extractTwitterArticle(tweetId);
      if (!body) {
        console.warn(`[extractTwitter] Apify returned no markdown for X Article ${finalUrl}; falling back to tweet metadata`);
        return null;
      }
      console.log(`[extractTwitter] Expanded t.co ${shortUrl} → X Article ${finalUrl} (${body.length} chars)`);
      return body;
    } catch (err) {
      console.warn(`[extractTwitter] Apify X Article extraction failed for ${finalUrl}: ${err.message}`);
      return null;
    }
  }
  ...
  try {
    ...
    // 451 = "unavailable for legal reasons"; Jina returns it for some hosts
    // that block third-party fetchers. It's an unfixable external block, so
    // degrade gracefully to a plain tweet save (return null) rather than
    // letting it bubble up and crash the pipeline.
...
async function extractTwitter(url) {
  ...
  // stays the caption.
  const articleBody = await resolveTcoArticle(text, id);
  if (articleBody) {
