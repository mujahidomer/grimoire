...
// Dedicated non-pooling HTTPS agent for the Anthropic client below. The SDK's
// built-in default is a SHARED, process-wide agentkeepalive agent
...
// gzip response stream was cut mid-flight, surfacing as
// ERR_STREAM_PREMATURE_CLOSE and failing every background classify call. With
// keepAlive:false, each request opens a fresh connection and closes it after,
// so there is no pooled socket that can go stale. Passing this via httpAgent to
// `new Anthropic({...})` scopes the change to THIS client only — the shared
// default agent is untouched, so other Anthropic SDK usage (e.g. the Haiku
// vision client in lib/extractor.js) is unaffected.
const anthropicHttpsAgent = new https.Agent({ keepAlive: false });

// Diagnostic wrapper around the SDK's own fetch (same node-fetch package it
// uses internally, via its shared module-level agentkeepalive HTTPS agent —
// see @anthropic-ai/sdk/_shims/node-runtime.js). Logs status + the headers
// that matter for a mid-stream failure (content-encoding/content-length/
// connection) as soon as they arrive, BEFORE the SDK reads/gunzips the body.
// Added to diagnose ERR_STREAM_PREMATURE_CLOSE crashes surfacing from the
// background save job: if a failure logs "200" here but the body read still
// throws, that points at the response stream getting cut off after Anthropic
// already answered (e.g. a stale reused keep-alive socket) rather than a
// request-side timeout/abort. Logging only — no retry/behavior change.
...
const client = new Anthropic({
  ...
  fetch: loggedFetch,
  // Overrides the SDK's shared keep-alive default with a fresh-connection agent
  // (see anthropicHttpsAgent above) to fix ERR_STREAM_PREMATURE_CLOSE.
