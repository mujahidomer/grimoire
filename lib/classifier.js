...
- CITATIONS AND FURTHER READING: a post, thread, article, essay, newsletter issue, or talk that the content cites as a source, credits, or points to for further reading. A "Sources", "References", "Further reading", or "As argued by..." list is not a list of entities. This covers social and blog-hosted writing especially — x.com/Twitter threads, LinkedIn and Threads posts, Medium and Substack essays, personal blog posts — and anything phrased in the "Author Name: Article Title" shape. The named tools, products, books, courses, and repos those pieces DISCUSS are still entities; the pieces themselves are not. A book, course, or code repository the content recommends you go get is still an entity even when it is cited — the test is whether the user would GO USE the thing or merely READ ABOUT the argument.
...

{
  ...
  "summary": "what this content covers — 1-2 sentences, 200 characters MAXIMUM. This is a hard limit: it renders as a 4-5 line blurb under the title, so a longer summary is a bug, not a bonus. Cut scene-setting and narration; say what the content is about and stop.",
...
// Entity-only extraction, for the linked-resource path.
//
// Deliberately reuses processContent instead of a bespoke entity prompt: the
// ENTITIES RULE lives inside SYSTEM_PROMPT, and a second copy of it would
// drift from the one every save uses — a dua extracted from a linked page
// would then look different from the same dua extracted from a save. The rest
// of the classification (title, summary, takeaways, category) is discarded:
// a linked resource is an extension of its parent save, not an item of its
// own, so only its digest entries are kept.
//
// Never throws — a page with nothing worth digesting is a normal outcome and
// must not fail the resource attach that triggered it.
async function extractEntities(text, sourceUrl, subcategoryVocab = {}) {
  if (!text || !text.trim()) return [];
  try {
    const [item] = await processContent(text, sourceUrl, [], {}, subcategoryVocab);
    return Array.isArray(item && item.entities) ? item.entities : [];
  } catch (err) {
    console.error(`[extractEntities] failed for ${sourceUrl}:`, err.message);
    return [];
  }
}
...
async function processContent(text, sourceUrl, hashtags, parts = {}, subcategoryVocab = {}) {
  ...
  // Then drop the save's own citations — runs after url verification so it
  // sees the surviving (real, in-content) urls it keys on.
  dropCitationEntities(item);
...
async function researchUrl(sourceUrl, subcategoryVocab = {}) {
  ...
  // The cited-source guard does apply: it keys on the entity's own shape, not on
  // whether the url appears in a source we hold, and a researched page cites
  // things just as readily as a saved one.
  dropCitationEntities(item);
  item.transcript = '';
  item.caption = '';
  item.fileName = generateFileName(item);
...
// Cited-source guard. The ENTITIES RULE tells the model not to extract a save's
// citations ("Sources", "Further reading", "As argued by…"), but a prompt rule
// alone drifts — and this one fails in a doubly-invisible way: a cited X/Medium
// post becomes a bare Digest row that can NEVER resolve a link either, because
// social links are not valid canonical URLs. So the class is also enforced here.
//
// Deliberately narrow: it fires only on the pairing of a writing-host URL with
// the "Author: Title" citation shape, so a GitHub repo, a course, or a product
// on the same host still survives. Bare-name entities are never touched — with
// no URL there is nothing to distinguish a citation from a real resource, and
// the prompt is the right place for that call.
const CITATION_HOSTS = [
  'x.com', 'twitter.com', 'linkedin.com', 'threads.net',
  'medium.com', 'substack.com', 'dev.to', 'hashnode.dev',
];

function isCitationEntity(entity) {
  if (!entity || typeof entity !== 'object') return false;
  if (entity.type !== 'resource' && entity.type !== 'article') return false;
  if (typeof entity.url !== 'string' || !entity.url) return false;

  let host;
  try {
    host = new URL(entity.url).hostname.toLowerCase().replace(/^www\./, '');
  } catch {
    return false;
  }
  const onWritingHost = CITATION_HOSTS.some(h => host === h || host.endsWith(`.${h}`));
  if (!onWritingHost) return false;

  // "Hamel Husain: Loop Engineering Is Dead", "Steinberger's loops-or-graphs
  // post" — a person attached to a piece of writing, which is what a citation
  // looks like and what a product name doesn't. The possessive arm requires
  // the writing noun too, so a publication that happens to be named
  // possessively ("Lenny's Newsletter" on Substack) isn't swept up.
  const name = String(entity.name || '');
  if (/:\s*\S/.test(name)) return true;
  return /['’]s\s+[\w-]*\s*(post|thread|article|essay|piece|writeup|write-up|take|blog|newsletter issue)\b/i.test(name);
}

function dropCitationEntities(item) {
  if (!Array.isArray(item.entities)) return;
  item.entities = item.entities.filter(entity => {
    if (!isCitationEntity(entity)) return true;
    console.warn(`[classifier] Dropping cited source, not an entity: ${entity.name} (${entity.url})`);
    return false;
  });
}
...
module.exports = { processContent, researchUrl, processLinkedResource, extractEntities, TOP_CATEGORY_TIEBREAK, isFailedCapture };
