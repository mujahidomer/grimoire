...

{
  ...
  "artifact_url_source": "content if the URL came explicitly from the transcript or content, ai_inferred if Haiku supplied it from training knowledge, null if artifact_url is null"
}
...
- Content is a cooking tutorial → artifact_type: none, artifact_name: null

Type priority order (use the first one that fits):
skill → tool → resource → person → concept → none

Populate artifact_url if a URL is explicitly present in the content (mark source: content) OR if you can confidently identify the artifact's primary URL from your training knowledge (mark source: ai_inferred). If genuinely unknown, null.
