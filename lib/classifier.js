...
9. Claude skills / tools / installable workflows: When artifact_type is "skill", output a top-level "skills" array in the JSON — a sibling of keyTakeaways, NOT nested inside it. Each entry is an object:
   - "name": the actual skill/tool name (e.g. "ReMotion", "Impeccable", "Frontend Design") — never the title of the video or reel
   - "what_it_does": one sentence, max 10 words
   - "install_command": the exact install or activation command if mentioned in the content, otherwise null
   - "source_url": GitHub repo, docs page, or product URL if mentioned in the content, otherwise null
   - Include every skill the content covers. If the content is about a concept or strategy that only mentions a tool in passing, do NOT include it — extract only skills that are the actual subject of the content.
   - Output "skills" ONLY when artifact_type is "skill". For all other artifact types, omit it entirely.
10. HIGHLIGHTS RULE (applies to ALL artifact types except "skill"):

After extracting all key takeaways, also output a top-level "highlights" array inside keyTakeaways. This is a distillation — not a copy — of the most important things a user would want to see at a glance without opening the full file.

Rules:
- 1 to 5 entries maximum. Pick only what genuinely matters.
- Each entry has exactly two fields:
    label: what this highlight is (e.g. "Hadith", "Core Insight", "Key Step", "Arabic Text", "Recipe Tip", "Warning", "Tool Used")
    value: the actual content — can be Arabic text, a quote, a sentence, a step. Keep it under 30 words.
- Do NOT copy full prose blocks or repeat the summary.
- Do NOT duplicate content already in the highlights array.
- For Islamic content: highlight the Arabic text + translation of the most important dua or hadith mentioned.
- For recipes: highlight the most important technique or step.
- For concepts: highlight the single core insight sentence.
- For tools: highlight the primary use case in one sentence.
- For workflows: highlight the most important step or outcome.
- For resources: highlight the top 2-3 items in the collection.
- For skill type: omit highlights entirely — skills array handles this.
- Invent the label freely — it should describe what the value is, not be a generic word like "highlight" or "point".
