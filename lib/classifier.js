...
// Cap the main content/transcript fed to Haiku at the first 1,500 characters,
// appending an ellipsis when anything was dropped. Metadata (URL, caption,
// author) is passed separately and is never truncated here.
const CLASSIFY_CONTENT_LIMIT = 1500;

function truncateForClassification(text) {
  const s = text || '';
  if (s.length <= CLASSIFY_CONTENT_LIMIT) return s;
  return s.substring(0, CLASSIFY_CONTENT_LIMIT) + '...';
}

async function processContent(text, sourceUrl, hashtags, parts = {}) {
  const truncated = truncateForClassification(text);
  // Keep the caption/title intact and pass it alongside the truncated content
  // so Haiku still has full context on what the post is about.
