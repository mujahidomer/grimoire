...
// Render the closed subcategory vocabulary (one line per top_category) for
// injection into the classifier prompt. `vocab` is { "<top_category name>":
// ["Sub A", "Sub B"] } as returned by repository.getSubcategoryVocabulary().
...
function renderSubcategoryVocab(vocab) {
  const lines = getTopCategories().map(c => {
    ...
    return `- ${c}: ${subs.length ? subs.join(' | ') : '(none yet — you may create the first)'}`;
  });
...
function reconcileSubcategory(item, vocab) {
  ...
  const existing = (vocab && vocab[item.top_category]) || [];
...

{
  ...
  "category": "the topic shelf this belongs on WITHIN top_category. EXISTING SUBCATEGORIES per top_category are listed below. For your chosen top_category, if any listed subcategory reasonably fits this content, return that name EXACTLY as written. Only if none reasonably fits may you propose a NEW short Title Case label (1-3 words, e.g. 'Prompt Engineering', 'Duas', 'Desserts'). Never invent a near-duplicate of an existing one.",
