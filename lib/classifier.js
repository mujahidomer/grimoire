...

{
  ...
  "artifact_url": "a direct URL to the artifact if one is explicitly present in the content, otherwise null"
}

keyTakeaways must have 3-5 items.

artifact_type identifies the single primary artifact the content is about:
- "skill": a named Claude skill, prompt technique, or AI workflow that can be downloaded or installed
- "tool": a named software tool, app, or service being demonstrated or reviewed
- "resource": a named paper, guide, template, or downloadable file
- "person": the content is primarily about or by a specific named individual (interview, profile, creator content)
- "concept": the content explains a named concept, framework, or idea with no associated artifact
- "none": cooking, lifestyle, general commentary, or anything with no primary artifact
Set artifact_name to null and artifact_url to null whenever artifact_type is "none". Only set artifact_url when an explicit URL to the artifact appears in the content; never invent one.
...

{
  ...
  "artifact_type": "exactly one of: skill | tool | resource | person | concept | none",
  "artifact_name": "the name of the primary artifact, or null if artifact_type is none",
  "artifact_url": "a direct URL to the artifact if one is explicitly present or found, otherwise null"
}
...
- "concept": the content explains a named concept, framework, or idea with no associated artifact
- "none": cooking, lifestyle, general commentary, or anything with no primary artifact
Set artifact_name to null and artifact_url to null whenever artifact_type is "none". Only set artifact_url to a URL you are confident points to the artifact; never invent one.
...
function normalizeArtifactFields(item) {
  ...
  const url = typeof item.artifact_url === 'string' ? item.artifact_url.trim() : '';
  item.artifact_url = url || null;
}
