...
// Same dataset's copies of the primary collections — used only as the
// full-text search corpus (number lookups for these go to the CDN with grades).
const SEARCH_ONLY_BOOKS = {
...
async function loadBook(key) {
  ...
  const file = EXTENDED_BOOKS[key]?.file || SEARCH_ONLY_BOOKS[key];
  if (!file) throw new Error(`unknown book "${key}"`);
  const data = await getJson(`https://cdn.jsdelivr.net/gh/AhmedBaset/hadith-json@main/db/by_book/${file}`);
  const rows = (data.hadiths || []).map(h => {
...
async function searchHadith(query, books) {
  const corpus = (Array.isArray(books) && books.length ? books : [
    ...Object.keys(SEARCH_ONLY_BOOKS), ...Object.keys(EXTENDED_BOOKS),
  ...
  for (const key of corpus) {
    if (!EXTENDED_BOOKS[key] && !SEARCH_ONLY_BOOKS[key]) continue;
