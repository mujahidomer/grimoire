...
function toItemRow(input, userId, source) {
  ...
  // write path (lib/drive.js) so both backends behave identically. The promoted
  // URL is still written to linked_resources by the caller — promotion only
  // populates artifact_url; it does not own the linked_resources table.
  if (artifactType !== 'none' && !row.artifact_url) {
    const candidates = (Array.isArray(input.linked_resources) ? input.linked_resources : [])
      ...
      .filter(Boolean);
    const primary = pickPrimaryArtifactLink(candidates);
    if (primary) {
