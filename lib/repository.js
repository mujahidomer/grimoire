...
function artifactTypeLabel(type) {
  return ARTIFACT_TYPE_LABELS[type] || (type.charAt(0).toUpperCase() + type.slice(1));
}
...
async function getDigest({ userId } = {}) {
  ...
  const { data, error } = await sb
    .from('items')
    .select('id, title, artifact_type, artifact_name, artifact_url, skills, source_url, date_saved, category, item_tags(confidence_pending, tags(name))')
    .eq('user_id', userId)
