...
const VALID_ARTIFACT_TYPES = ['skill', 'tool', 'concept', 'workflow', 'recipe', 'islamic', 'resource', 'person', 'none'];
...
// Accept the skills array for the jsonb column. Returns a non-empty array of
// skill objects, or null when there's nothing usable (so the column stays null
// for the common case rather than storing []).
function normalizeSkills(value) {
  if (!Array.isArray(value)) return null;
  const cleaned = value.filter(s => s && typeof s === 'object' && !Array.isArray(s) && s.name);
...
function toItemRow(input, userId, source) {
  ...
  const row = {
    ...
    data_version: pick(input, 'data_version') ?? 'v2',
    subcategory: pick(input, 'subcategory') ?? null,
    // skills: jsonb array of { name, what_it_does, install_command, source_url },
    // populated only for skill-type saves; null otherwise.
    skills: artifactType === 'skill' ? normalizeSkills(pick(input, 'skills')) : null,
...
// Human-readable group labels per artifact_type. Anything not listed falls back
// to a title-cased version of the raw type.
const ARTIFACT_TYPE_LABELS = {
  skill: 'Skills',
  tool: 'Tools',
  concept: 'Concepts & Strategies',
  workflow: 'Workflows',
  recipe: 'Recipes',
...
function artifactTypeLabel(type) {
  return ARTIFACT_TYPE_LABELS[type] || String(type || '')
    .replace(/[_-]+/g, ' ')
    .replace(/\b\w/g, c => c.toUpperCase());
}

// Library digest: every structured artifact a user has saved, grouped by
// artifact_type. Excludes artifact_type='none' (raw saves with no artifact).
// Groups are sorted by item count desc; items within a group by date_saved desc.
async function getDigest({ userId } = {}) {
  const sb = getSupabase();
  const { data, error } = await sb
    .from('items')
    .select('id, title, artifact_type, artifact_name, artifact_url, skills, key_takeaways, source_url, date_saved, category, data_version, subcategory, item_tags(confidence_pending, tags(name))')
    .eq('user_id', userId)
    .neq('artifact_type', 'none')
    .order('date_saved', { ascending: false });
  if (error) throw new Error(`getDigest failed: ${error.message}`);

  const byType = new Map();
  for (const row of data || []) {
    const item = shapeItem(row);
    if (!byType.has(item.artifact_type)) byType.set(item.artifact_type, []);
    byType.get(item.artifact_type).push(item);
  }

  const groups = [...byType.entries()]
    .map(([type, items]) => ({ type, label: artifactTypeLabel(type), items }))
