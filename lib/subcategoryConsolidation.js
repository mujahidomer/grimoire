...
const { TOP_CATEGORIES, normalizeTopCategory } = require('./topCategories');
...
async function applyMerge(userId, topCategory, merge) {
  ...
  const { data, error } = await sb
    .from('items')
    .update({ category: merge.to })
    .eq('user_id', userId)
    .eq('top_category', topCategory)
    .in('category', merge.from)
    .eq('category_manually_set', false)
    .select('id');
...
// label strings (user-scoped, manual-skip, and `.in('category', from)` only
...
async function consolidateSubcategories({ userId, topCategory, apply = false, groups: submittedGroups } = {}) {
  ...
  const categoriesToCheck = topCategory ? [topCategory] : TOP_CATEGORIES;
