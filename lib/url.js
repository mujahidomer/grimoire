...
//   0 — bare IP, localhost, a URL that fails to parse, or a social-aggregator
//       post URL (twitter/x/instagram/tiktok/facebook/threads on a post path) —
//       these are typically the saved item's own source_url, not an artifact link
const SOCIAL_AGGREGATOR_HOSTS = new Set([
  'twitter.com', 'x.com', 'instagram.com', 'tiktok.com', 'facebook.com', 'threads.net'
]);
// Post-shaped path segments across those hosts: tweets (/status/), IG posts
// (/p/) and reels (/reel(s)/), TikTok videos (/video/), FB posts (/posts/),
// and Threads posts (/post/).
const SOCIAL_POST_PATH = /\/(status|reel|reels|p|post|posts|video|videos)\//;
...
function primaryLinkRank(url) {
  ...
  // Google Docs/Drive share links start with "docs." but are not documentation —
  // they're the content artifact itself. Drop to rank 1 so they don't outrank a
  // real repo or docs site, before the rank-3 prefix rule below catches them.
  if (host === 'docs.google.com' || host === 'drive.google.com') return 1;
  ...
  // Non-promotable: a social-aggregator post URL (tweet/reel/post/video). These
  // are the saved item's own source_url, never a useful artifact target.
  const bareHost = host.replace(/^m\./, '');
  if (SOCIAL_AGGREGATOR_HOSTS.has(bareHost) && SOCIAL_POST_PATH.test(path)) return 0;
